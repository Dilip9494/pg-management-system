...
async function loadDashboard() {
    try {
        ...
        allGuests = allGuests.map(updatePaymentStatus);
        ...
        const pendingGuests = activeGuests.filter(g => g.monthlyPaymentStatus === 'Pending' || g.monthlyPaymentStatus === 'Partial Paid').length;
        ...
        document.getElementById('b1Pending').textContent = b1Guests.filter(g => g.monthlyPaymentStatus === 'Pending' || g.monthlyPaymentStatus === 'Partial Paid').length;
        document.getElementById('b1Breached').textContent = b1Guests.filter(g => g.monthlyPaymentStatus === 'Breached').length;
        ...
        document.getElementById('b2Pending').textContent = b2Guests.filter(g => g.monthlyPaymentStatus === 'Pending' || g.monthlyPaymentStatus === 'Partial Paid').length;
        document.getElementById('b2Breached').textContent = b2Guests.filter(g => g.monthlyPaymentStatus === 'Breached').length;
        ...
        document.getElementById('recentActivity').innerHTML = '<p class="text-center text-danger">Error loading data</p>';
    }
}
...
function loadRecentActivity() {
    ...
    // Sort guests by upcoming due date
    ...
    sortedGuests.forEach(guest => {
        ...
        const statusClass =
            guest.monthlyPaymentStatus === 'Breached'
                ? 'danger'
                : guest.monthlyPaymentStatus === 'Pending'
                ? 'warning'
                : 'success';
        html += `
